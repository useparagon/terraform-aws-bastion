locals {
  ...
  lb_name = length(local.nlb_name_raw) <= 32 ? local.nlb_name_raw : "${regex_replace(substr(local.nlb_base, 0, 32 - length(local.lb_suffix)), "-+$", "")}${local.lb_suffix}"

  lb_target_group_name = length(local.nlb_target_name_raw) <= 32 ? local.nlb_target_name_raw : "${regex_replace(substr(local.nlb_base, 0, 32 - length(local.tg_suffix)), "-+$", "")}${local.tg_suffix}"
